...
git config $scope diff.dmfo.command "powershell.exe -File $InstallPath/ps1/dmfo-diff.ps1"
git config $scope diff.dmfo.binary "true"

# Register Merge Driver
git config $scope merge.dmfo.name "DMFO merge driver"
git config $scope merge.dmfo.driver "powershell.exe -File $InstallPath/ps1/dmfo-merge.ps1 %O %A %B %L %P"
